#requires -Version 7
# test.dp-onnx.model-db-chunking.ps1 — Receipt for CRQ191 ONE MODEL, ONE DB.
...
#   Dogfood:  ss run tests/test.dp-onnx.model-db-chunking.ps1
...
# In-proc head types (this test runs inside the ss.exe runspace, same as bench.dp-onnx.model-db-gather.ps1).
...
[pscustomobject]@{
    test='test.dp-onnx.model-db-chunking'; pass=$pass
