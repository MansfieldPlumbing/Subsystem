#requires -Version 7
# test.dp-onnx.onnxwire.ps1 — Receipt for the home-rolled ONNX protobuf reader/writer (OnnxWire.cs).
# OnnxWire replaces Google.Protobuf + Grpc.Tools so dp-onnx folds into the in-proc Roslyn build with no
...
#   Dogfood:  ss run tests/test.dp-onnx.onnxwire.ps1
...
# (1) roundtrip: graph build -> OnnxWire.ToByteArray -> OnnxWire.Parser.ParseFrom -> interp run.
...
# (2) real-model parse: walk the full graph + read initializers/attributes through OnnxWire.
...
if ($model) {
    ...
    Assert ($nodes -gt 1000)      "real model parsed: $nodes nodes off OnnxWire (no protobuf)"
...
if ($fails.Count -eq 0) { Write-Host "PASS  test.dp-onnx.onnxwire  (Google.Protobuf + Grpc.Tools dropped; reader/writer verified)" -ForegroundColor Green }
else { Write-Host "FAIL  test.dp-onnx.onnxwire  ($($fails.Count) failed)" -ForegroundColor Red; exit 1 }
