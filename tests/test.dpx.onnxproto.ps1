#requires -Version 7
# test.dpx.onnxproto.ps1 — Receipt for the home-rolled ONNX protobuf reader/writer (OnnxProto.cs).
# OnnxProto replaces Google.Protobuf + Grpc.Tools so dpx folds into the in-proc Roslyn build with no
...
#   Dogfood:  ss run tests/test.dpx.onnxproto.ps1
...
[pscustomobject]@{ test='test.dpx.onnxproto'; pass=$pass; verdict=$(if($pass){'home-rolled ONNX protobuf reader/writer; write->read->run roundtrip bit-exact'}else{'see failures'}) }
