#requires -Version 7
# test.dpx.gemma4-probe.ps1 — Receipt for reading the Gemma-4 E2B model on dpx, sovereignly.
# The live model is gemma-4-E2B-it.litertlm (a LITERTLM container of .tflite FlatBuffers). dpx reads it
# with the home-rolled FlatBuffers reader (LiteRt.cs) — no FlatBuffers lib, no LiteRT lib — translating to the
# same Onnx.ModelProto IR so `probe` enumerates the op set and maps it onto the dpx kernels. The decomposed
...
#   Dogfood:  ss run tests/test.dpx.gemma4-probe.ps1
...
# Locate the engine the same way the other dpx receipts do: the build/ sibling, freshest dpx.exe.
...
if (-not $exe) { Write-Host "SKIP - dpx.exe not built (build src\native\dpx\onnx-interp\Onnx.Interp.csproj)." -ForegroundColor Yellow; return }
...
Assert ($cov -and $cov.mapped -ge 20) "op set maps onto dpx kernels: $(if($cov){"$($cov.mapped)/$($cov.distinct)"}else{'0/0'})"
# Slice 1 (the op gap): the quant/norm ops the transformer graph needs now map to real dpx kernels.
$nowMapped = @('DEQUANTIZE','QUANTIZE','RSQRT','NOT_EQUAL') | Where-Object { $pr -notmatch ($_ + '\s.*MISSING') }
Assert ($nowMapped.Count -eq 4) "DEQUANTIZE/QUANTIZE/RSQRT/NOT_EQUAL map to dpx kernels (mapped: $($nowMapped -join ','))"
...
Write-Host "  gemma-4 E2B slice 2: section [2] embedder ran $ran nodes -> $rOuts finite output(s) on dpx"
...
Assert ($r5Finite)    "section [5] output is finite — a real gemma compute block runs on dpx, not just the embedder"
Write-Host "  gemma-4 E2B slice 2b: section [5] RMSNorm+FC ran $ran5 nodes finite on dpx"

$pass = $fails.Count -eq 0
Write-Host ""
Write-Host ($(if($pass){"PASS - dpx reads the gemma-4 E2B .litertlm sovereignly (no FlatBuffers lib, no LiteRT lib); op coverage measured."}else{"FAIL ($($fails.Count)): $($fails -join '; ')"})) -ForegroundColor $(if($pass){'Green'}else{'Red'})
[pscustomobject]@{ test='test.dpx.gemma4-probe'; pass=$pass; sections=$sections; spTokenizer=$hasSP; coverage=$(if($cov){"$($cov.mapped)/$($cov.distinct)"}); ops=$(if($cov){$cov.ops}); verdict=$(if($pass){'gemma-4 E2B read off the sovereign litertlm/tflite reader; coverage measured'}else{'see failures'}) }
