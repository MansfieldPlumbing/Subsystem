#requires -Version 7
# test.dpx.sentencepiece.ps1 — Receipt for the home-rolled SentencePiece reader + Unigram tokenizer
...
#   Dogfood:  ss run tests/test.dpx.sentencepiece.ps1
...
[pscustomobject]@{ test='test.dpx.sentencepiece'; pass=$pass; vocab=$vocab; bos=$bos; eos=$eos; pad=$pad; verdict=$(if($pass){'home-rolled SentencePiece reader + Unigram tokenizer; gemma-4 vocab + control tokens + roundtrip proven'}else{'see failures'}) }
