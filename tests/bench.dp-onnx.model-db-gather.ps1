#requires -Version 7
# bench.dp-onnx.model-db-gather.ps1 — Real numbers for the model.db -> VOM-region gather (two-tier:
...
#   Dogfood:  ss run tests/bench.dp-onnx.model-db-gather.ps1
$ErrorActionPreference='Stop'
$db = @($env:SS_MODEL_DB,'S:\modeldb\demucsv4.db','S:\modeldb\kokoro.db','S:\modeldb\gemma-4-e2b.db') |
      Where-Object { $_ -and (Test-Path $_) } | Select-Object -First 1
if(-not $db){ Write-Host "SKIP - no model.db present (dp-onnx db <model.onnx> S:\modeldb\<name>.db)." -ForegroundColor Yellow; return }
...
[pscustomobject]@{
    bench='dp-onnx.model-db-gather'; db=(Split-Path $db -Leaf)
