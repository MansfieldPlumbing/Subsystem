#requires -Version 7
# test.gpu.dpgpu-gemm-csharp.ps1 — Receipt for dp-onnx's PURE-C# D3D12 GPU seam (src/native/dp-onnx/onnx-interp/GpuD3D12.cs).
# The engine's `Gpu.dpgpu_gemm` no longer P/Invokes a native dpgpu.dll — it dispatches D3D12 compute from managed
# code (COM vtable interop + serialized root sig), reproducing dpgpu.cpp's GEMM. This builds dp-onnx with dotnet,
# compiles gemm.hlsl -> DXIL with dxc, runs `dp-onnx gpu-test <dxil>`, and asserts MATCH vs the CPU MatMul AND that
...
# resolve the dp-onnx project across boxes (laptop S:\subsystem, workstation S:\subsystem-project\subsystem-main, or share)
...
if (-not $dp) { Write-Host "SKIP - dp-onnx project not found on this box." -ForegroundColor Yellow; return }
...
# 2. build dp-onnx (managed; NO native dpgpu.dll)
$csproj = Join-Path $dp 'onnx-interp\Onnx.Interp.csproj'
$build = & $dotnet build $csproj -c Release --nologo 2>&1 | Out-String
Assert ($build -match 'Build succeeded' -or $build -notmatch 'error ') "dp-onnx builds with the pure-C# GPU seam"
$exe = Get-ChildItem 'S:\build\Onnx.Interp\bin\Release\net*\dp-onnx.exe','S:\subsystem\src\native\dp-onnx\onnx-interp\bin\Release\net*\dp-onnx.exe' -EA SilentlyContinue | Select-Object -First 1
if (-not $exe) { Write-Host "SKIP - built dp-onnx.exe not found (output redirect)." -ForegroundColor Yellow; return }
...
Write-Host ""
Write-Host ($(if($pass){"PASS - dp-onnx's GPU seam is pure-C# (D3D12 + Vulkan, no dpgpu.dll / no MSVC) and both match the CPU GEMM."}else{"FAIL ($($fails.Count)): $($fails -join '; ')"})) -ForegroundColor $(if($pass){'Green'}else{'Red'})
