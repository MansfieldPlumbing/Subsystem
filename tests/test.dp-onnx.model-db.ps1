#requires -Version 7
# test.dp-onnx.model-db.ps1 — Receipt for the models-as-.db converter + op triage as a query.
# `dp-onnx db` compiles an ONNX graph into a queryable SQLite model store (6 tables); `dp-onnx db-stats`
...
#   Dogfood:  ss run tests/test.dp-onnx.model-db.ps1
...
if (-not $exe) { Write-Host "SKIP - dp-onnx.exe not built." -ForegroundColor Yellow; return }
