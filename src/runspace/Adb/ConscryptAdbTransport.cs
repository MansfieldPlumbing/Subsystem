...
public sealed class ConscryptAdbTransport : IAdbTransport
{
    ...
    private sealed class DuplexStream : Stream
    {
        ...
        protected override void Dispose(bool disposing) { if (disposing) { try { _in.Dispose(); } catch (Exception ex) { Dg.Warn("adb-tls", ex); } try { _out.Dispose(); } catch (Exception ex) { Dg.Warn("adb-tls", ex); } } }
    }
}
