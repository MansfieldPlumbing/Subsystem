...
// DpxAoa — Android AOA driver, the device-mode + host-mode counterpart to windows/DpWinUsb.cs (CRQ185
...
internal sealed class DpAoaDevice : IDisposable
{
    ...
    public void SendFrame(ulong fence, int x, int y, int w, int h, byte[] bgra)
    {
        ...
        byte[] desc = new byte[DpxAoa.DescriptorSize];
        DpxAoa.WriteDescriptor(desc, fence, x, y, w, h, bgra.Length);
        try
        {
            _tx.Write(desc);
            _tx.Write(bgra);
    ...
    private void ReceiveLoop()
    {
        ...
        while (Volatile.Read(ref _stopped) == 0)
        {
            try
            {
                if (!ReadFull(_rx!, desc, DpxAoa.DescriptorSize)) break;
    ...
    public void Stop()
    {
        ...
        try { _rx?.Close(); } catch (Exception ex) { Dg.Warn("dp-aoa", ex); }
        try { _tx?.Close(); } catch (Exception ex) { Dg.Warn("dp-aoa", ex); }
        try { _pfd?.Close(); } catch (Exception ex) { Dg.Warn("dp-aoa", ex); }
    }
...
internal sealed class DpAoaHost : IDisposable
{
    ...
    public static DpAoaHost? Open(Context ctx, UsbDevice device, out string? error)
    {
        ...
        if (device.VendorId != DpxAoa.AoaVid || device.ProductId != DpxAoa.AoaPidAcc)
        {
    ...
    private static bool SwitchToAoa(UsbManager manager, UsbDevice device, out string? error)
    {
        ...
        try
        {
            ...
            int got = conn.ControlTransfer((UsbAddressing)0xC0, DpxAoa.AoaGetProtocol, 0, 0, proto, 2, 5000);
            if (got < 2 || proto[0] < 1) { error = "AOA not supported by peer (protocol < 1)"; return false; }

            var strings = DpxAoa.IdentityStrings(device.DeviceName ?? "");
            for (int idx = 0; idx < strings.Length; idx++)
            {
                byte[] b = System.Text.Encoding.UTF8.GetBytes(strings[idx] + "\0");
                conn.ControlTransfer((UsbAddressing)0x40, DpxAoa.AoaSendString, 0, idx, b, b.Length, 5000);
            }

            conn.ControlTransfer((UsbAddressing)0x40, DpxAoa.AoaStartAccessory, 0, 0, null, 0, 5000);
            return true;
        }
        finally { conn.Close(); }
    }
    ...
    private void ReceiveLoop()
    {
        ...
        while (Volatile.Read(ref _stopped) == 0)
        {
            try
            {
                ...
                if (got != DpxAoa.DescriptorSize) continue;   // timeout/short read — latest-wins, just retry
    ...
    public void Stop()
    {
        if (Interlocked.Exchange(ref _stopped, 1) != 0) return;
        Vom.Vom.Terminate(_owner);
        try { _conn.Close(); } catch (Exception ex) { Dg.Warn("dp-aoa", ex); }
    }
