...
// deterministically (Reclaim = unmap -> dp12_close -> shutdown), the let-it-crash kill path.
//
...
public static class Surface
{
    ...
    {
        if (!DirectPortNative.dp12_init())
        {
            Console.Error.WriteLine("ss surface: dp12_init failed (no D3D12 device / directport.dll not found)");
            return 1;
        }

        long luidRaw = 0;
        DirectPortNative.dp12_get_adapter_luid(out luidRaw);
        ...
        IntPtr dp = DirectPortNative.dp12_create_shared_resource(
            (uint)width, (uint)height, (int)DirectPortNative.DpFormat.Video, false, texName, fenceName);
        if (dp == IntPtr.Zero)
        {
            Console.Error.WriteLine($"ss surface: dp12_create_shared_resource failed (HRESULT 0x{DirectPortNative.dp12_last_hresult():X8}, adapter UMA={DirectPortNative.dp12_is_uma()})");
            DirectPortNative.dp12_shutdown();
            return 1;
        }

        // A discrete-GPU shared texture lives in VRAM and can't be CPU-mapped. Draw the pattern into a
        // native scratch buffer and upload it each frame (dp12_upload_bgra does the GPU copy + fence).
        ...
        if (map == null)
        {
            ...
            DirectPortNative.dp12_unmap_memory(dp);
            DirectPortNative.dp12_close(dp);
            DirectPortNative.dp12_shutdown();
            return 1;
        }
        ...
        Vom.Vom.RegisterNative(owner, "DirectPortSurface", dp, reclaim: () =>
        {
            ...
            try { DirectPortNative.dp12_close(dpRef); } catch { }
            try { DirectPortNative.dp12_shutdown(); } catch { }
        ...
        while (!stop.IsSet)
        {
            ...
            frame++;
            if (!DirectPortNative.dp12_upload_bgra(dp, scratch, rowPitch, (uint)width, (uint)height, frame))
            { Console.Error.WriteLine("ss surface: dp12_upload_bgra failed"); break; }   // GPU copy + shared-fence signal
