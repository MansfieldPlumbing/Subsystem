...
internal static class Shim
{
    ...
    // the CodeContext cmdlets live in their own assembly; the request/EOS cmdlets (Remedy-*Request /
    // Get-Request / Close-Request / Add-EosLog) live in THIS host assembly. Dedup by command name. Additive: a load failure is reported,
    ...
    static string? RefuseStringy(string[] args)
    {
        ...
        string[] verbs = { "git", "build", "check", "contextualize", "onboard", "diag", "selftest", "test",
