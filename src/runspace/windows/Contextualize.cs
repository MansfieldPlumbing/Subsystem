...
// `ss contextualize` (-c) — the impeccable self-description. A cold agent (or Scott, or a blind user) must
// be able to understand this system AND its source FROM THE BINARY, without re-ingesting the tree or
// reaching for memory. ss.exe embeds the CONTRACT (SystemCatalog.json — data, gated, can't rot); the
// architecture MAP is no longer an embedded doc — `ss contextualize --map` re-derives it LIVE from the
// source tree beside the binary (LiveMap), so it can never go stale. This is also the seed of the MCP
// tools/list surface ("one JSON, N consumers").
internal static class Contextualize
{
    public static int Run(string[] args)
    {
        ...
        string path = ArgValue(args, "--path") ?? ArgValue(args, "-p") ?? Environment.CurrentDirectory;   // default = the current working dir (scanned recursively); -p targets another dir/drive, intentional only
    ...
    // `--path <dir>` value lookup (for `ss contextualize --map --path <repo>`).
    ...
    private static void PrintHuman(string? catalogJson)
    {
        ...
        Console.WriteLine("\"It's NT, and it's a fractal.\"");
        Console.WriteLine();
        ...
        Console.WriteLine("  ss contextualize --map    architecture, subsystem by subsystem (the deep map)");
        Console.WriteLine("  ss contextualize --json   the contract, structured (for agents / MCP)");
