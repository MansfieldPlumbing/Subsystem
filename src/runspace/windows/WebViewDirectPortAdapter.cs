...
internal sealed class WebViewDirectPortAdapter : IDisposable
{
    ...
    public void SignalFrame(ReadOnlySpan<float> data)
    {
        ...
        try { _web.CoreWebView2.PostWebMessageAsString("frame:" + fv); } catch (Exception ex) { Dg.Warn("webviewport", ex); }
    }

    public void Dispose()
    {
        try { _buffer?.Close(); } catch (Exception ex) { Dg.Warn("webviewport", ex); }
        _buffer = null;
    }
