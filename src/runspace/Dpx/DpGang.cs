// DpGang.cs - the brutally-synchronous worker-gang primitive (CRQ195, Scott's design). "we do not
// async... brutal synchrony... call it DpGang". N lanes move in LOCKSTEP through phases gated by
...
// alignment every VOM handle already gets via Vom.Alloc; DpGang invents no new alignment).
//
...
public sealed class DpGangHazardException : InvalidOperationException
{
    ...
    public DpGangHazardException(int role, uint priorHandleId, int observedRefCount)
        : base($"DpGang role {role}: refused to rebind handle 0x{priorHandleId:X8} while refcount={observedRefCount} (>1 floor) - a straggler lane may still be reading through the old alias.")
    {
...
public readonly struct DpGangBuffer
{
    ...
    // Read* (verb catalog "Read", matching DpTensor.ReadF32/ReadBf16/ReadBytes) - resolves the
...
// level (that is a within-phase fan-out, not gang advancement) - DpGang itself never does.
public sealed class DpGang : IDisposable
{
    private readonly Owner _owner;
    ...
    public DpGang(Owner parentOwner, int laneCount, int roleCount = 0)
    ...
    // Dp.MatMulNBits that stands a gang up for one call and tears it down when done. rootPath is the
    ...
    public DpGang(string rootPath, int laneCount, int roleCount = 0)
    {
    ...
    public DpGangBuffer[] AllocResidentBuffers(int elementsPerBuffer, VomFormat format = VomFormat.Float32)
    {
        ...
        {
            var h = VomClass.Alloc(_owner, elementsPerBuffer * elemBytes, format, "DpGang.Resident", name: $"role{r}");
    ...
    private void ThrowFirstFault()
    {
        for (int i = 0; i < LaneCount; i++)
            if (_err[i] != null) { var e = _err[i]; _err[i] = null; throw new AggregateException($"DpGang lane {i} faulted", e); }
    }
