// DpxTensor.cs - the VOM-native tensor (CRQ164). A Handle (+ optional Fence), resolved to a Span
...
public readonly struct DpxTensor
{
    ...
    private DpxTensor(Owner owner, Handle data, int[] shape, Handle? qScale, Handle? qZero, int qbits, int qaxis)
    {
    ...
    private static int ElemBytes(VomFormat f) => f switch
    {
        ...
        VomFormat.Int64  => 8,
        VomFormat.Bytes  => 1,
        _ => throw new ArgumentOutOfRangeException(nameof(f), f, "unhandled VomFormat in DpxTensor.ElemBytes"),
    };

    // --- Allocation ---

    public static DpxTensor Alloc(Owner owner, int[] shape, VomFormat format = VomFormat.Bf16,
                                  bool withFence = false, string subdir = "Objects", string? name = null)
    {
        ...
        var h = VomClass.Alloc(owner, byteCount, format, "DpxTensor", withFence, subdir, name);
        return new DpxTensor(owner, h, shape, null, null, 0, 0);
    }
    ...
    public static DpxTensor AllocBlobAhb(Owner owner, int[] shape, int byteCount, out IntPtr ahb,
                                        string subdir = "Weights", string? name = null)
    {
        var (buf, mapped) = AhbNative.AllocBlob(byteCount);
        var h = VomClass.RegisterNative(owner, "DpxTensor.Packed.Ahb", mapped,
    ...
    public static DpxTensor AllocQuant(Owner owner, int[] shape, int packedByteCount, int qbits, int qaxis,
                                       int scaleCount, bool hasZero, string subdir = "Objects", string? name = null)
    {
        var data = VomClass.Alloc(owner, packedByteCount, VomFormat.Bytes, "DpxTensor.Packed", false, subdir, name);
        var scale = VomClass.Alloc(owner, checked(scaleCount * ElemBytes(VomFormat.Bf16)), VomFormat.Bf16, "DpxTensor.QScale", false, subdir, name is null ? null : name + ".qscale");
        Handle? zero = hasZero
            ? VomClass.Alloc(owner, checked(scaleCount * ElemBytes(VomFormat.Bf16)), VomFormat.Bf16, "DpxTensor.QZero", false, subdir, name is null ? null : name + ".qzero")
            : null;
        return new DpxTensor(owner, data, shape, scale, zero, qbits, qaxis);
    }
    ...
    public DpxTensor Resolve(Owner scratchOwner, string? name = null)
    {
        if (Data.Format != VomFormat.Bf16)
    ...
    public unsafe void WriteTo(DpxTensor dest)
    {
    ...
    {
        var f = GetFence() ?? throw new InvalidOperationException("DpxTensor was not allocated withFence:true.");
        f.Signal(value);
    }
