...
export class Shell {
  ...
  async boot() {
    ...
    this._bindCharmsGesture();   // left-edge swipe-right opens Charms (owner directive 2026-06-19)
  ...
  // Charms gesture (owner directive 2026-06-19): a LEFT-edge swipe-right opens the Charms bar, always,
  // within the app. Mirrors _bindBackGesture (right edge). The left edge used to fall through to the OS
  // predictive-back — redundant with the right-edge back — so we claim it for Charms instead.
  _bindCharmsGesture() {
    // A LEFT-edge gutter overlay ABOVE the presenter iframes. Touches that land on an iframe never reach
    // this.root, so a root listener never fires over the desktop — the gutter sits on top and catches the
    // edge swipe regardless. A short rightward swipe from the gutter opens Charms.
    const EDGE = 20, THRESH = 40;
    const g = document.createElement('div');
    g.className = 'charms-gutter';
    g.style.cssText = 'position:fixed;left:0;top:0;width:' + EDGE + 'px;height:100%;z-index:2147483646;background:transparent;';
    this.root.appendChild(g);
    let sx = 0, sy = 0;
    g.addEventListener('touchstart', (e) => { const t = e.touches[0]; if (!t) return; sx = t.clientX; sy = t.clientY; }, { passive: true });
    g.addEventListener('touchend', (e) => {
      const t = e.changedTouches[0]; if (!t) return;
      const dx = t.clientX - sx, dy = t.clientY - sy;
      if (dx > THRESH && Math.abs(dx) > Math.abs(dy)) this.toggleCharms();
    }, { passive: true });
  }
