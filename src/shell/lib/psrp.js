 ...
 *
 * The renderer can't speak MS-PSRP itself; this talks to the /psrp seam, which brokers
...
async function post(path, payload) {
  // The /psrp seam is cap-gated like every command lane — without the per-boot cap header the host
  // returns 401, which is exactly why files + edit were dead. Mirror the surface/api bridge.
  const res = await fetch(path, {
    method: 'POST',
    headers: { 'X-Subsystem-Cap': (window.AndroidBridge?.getCap?.() || window.__ssCap || '') },
