"""
export_gemma4_e2b_decomposed.py — Gemma 4 E2B → DECOMPOSED ONNX for dpx.
...
ops land on dpx's existing ~70-op set (MatMul, Gelu, Pow/ReduceMean/Sqrt/Div/Mul = RMSNorm,
Sin/Cos/Slice/Neg/Concat = RoPE, Softmax, Gather, Tanh = softcap), so NO new dpx handlers
...
# diff(x, n=1, dim) = x[1:] - x[:-1] = Slice + Sub (both native in dpx).
...
# Load NATIVE bf16 (~10 GB), not fp32 (~20 GB). dpx kernels are fp32: either (a) cast weights with
# .float() before export → fp32 ONNX ~20 GB on W: (needs ~20 GB RAM), or (b) keep bf16 ONNX (~10 GB) and
# add a bf16→fp32 cast-on-load in dpx. Decide at the export/probe step; default here = native bf16.
...
print("\nNEXT:  dpx probe <onnx>   then   dpx run <onnx> --inputs <dir>   then diff logits vs llama.cpp")
