# dp-onnx — the home-rolled, ORT-free ONNX engine (vendored)
...
- `onnx-interp/Program.cs` + `onnx-interp/Onnx.Interp.csproj` — the interpreter (`AssemblyName=dp-onnx`).
...
## Build
```powershell
pwsh -File build.ps1     # 1) onnxnet -> Onnx.dll   2) onnx-interp -> dp-onnx.exe
```
...
- `io/kokoro_compiled.cs` — **emit output**, not source (regenerate with `dp-onnx emit`/`fold`).
