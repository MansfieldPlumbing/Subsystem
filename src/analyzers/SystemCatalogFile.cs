...
namespace Subsystem.Analyzers
{
    ...
    internal sealed class SystemCatalogFile
    {
        ...
        public readonly HashSet<string> BannedStringLiterals = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        ...
        private static SystemCatalogFile? Parse(string json)
        {
            ...
            if (Get(root, "banned") is Dictionary<string, object?> banned)
            {
                ...
                foreach (var s in Strings(Get(banned, "stringLiterals"))) c.BannedStringLiterals.Add(s);
            }
