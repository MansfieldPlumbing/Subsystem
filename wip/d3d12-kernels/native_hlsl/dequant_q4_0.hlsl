...
ByteAddressBuffer _79 : register(t0, space0);
...
void comp_main()
{
    ...
    uint q_idx = 8u * il;
    uint b_idx = ((1024u * i) + (32u * ir)) + q_idx;
    float d = float(_79.Load<half>(ib * 18 + 0));
    [unroll]
    for (uint l = 0u; l < 8u; l++)
    {
        _97.Store<half>(((b_idx + l) + 0u) * 2 + 0, half(d * (float(int(uint(_79.Load<???>(ib * 18 + (q_idx + l) * 1 + 2))) & 15) - 8.0f)));
        _97.Store<half>(((b_idx + l) + 16u) * 2 + 0, half(d * (float(_79.Load<???>(ib * 18 + (q_idx + l) * 1 + 2) >> 4) - 8.0f)));
    }
}
