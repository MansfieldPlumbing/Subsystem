...
RWByteAddressBuffer _182;
ByteAddressBuffer _187;
ByteAddressBuffer _243;
cbuffer parameter
{
...
void comp_main()
{
    ...
    if (p_mode == 0u)
    {
        ...
        uint idx = src_idx;
        float param = float(_187.Load<half>(idx * 2 + 0));
        float param_1 = float(_187.Load<half>((idx + offset) * 2 + 0));
        _182.Store<half>(dst_idx * 2 + 0, half(op(param, param_1)));
    }
    else
    {
        if (p_mode == 1u)
        {
            ...
            uint idx_1 = src_idx;
            float param_2 = float(_187.Load<half>((idx_1 + offset_1) * 2 + 0));
            float param_3 = float(_187.Load<half>(idx_1 * 2 + 0));
            _182.Store<half>(dst_idx * 2 + 0, half(op(param_2, param_3)));
        }
        else
        {
            uint idx_2 = src_idx;
            float param_4 = float(_187.Load<half>(idx_2 * 2 + 0));
            float param_5 = float(_243.Load<half>(idx_2 * 2 + 0));
            _182.Store<half>(dst_idx * 2 + 0, half(op(param_4, param_5)));
        }
    }
}
