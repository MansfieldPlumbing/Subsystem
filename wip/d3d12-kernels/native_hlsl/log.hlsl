...
ByteAddressBuffer _299;
RWByteAddressBuffer _312;
cbuffer parameter
{
...
void comp_main()
{
    ...
    uint param = idx;
    float val = float(_299.Load<half>((get_aoffset() + src0_idx(param)) * 2 + 0));
    uint param_1 = idx;
    _312.Store<half>((get_doffset() + dst_idx(param_1)) * 2 + 0, half(log(val)));
}
