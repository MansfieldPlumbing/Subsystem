...
ByteAddressBuffer _80 : register(t0, space0);
...
void comp_main()
{
    ...
    float d = float(_80.Load<half>(ib * 22 + 0));
    uint qh = (uint(_80.Load<uint16_t>(ib * 22 + 4)) << uint(16)) | uint(_80.Load<uint16_t>(ib * 22 + 2));
    uint q_idx = 8u * il;
    [unroll]
    for (uint l = 0u; l < 8u; l++)
    {
        uint iqs = q_idx + l;
        uint vui = uint(_80.Load<???>(ib * 22 + iqs * 1 + 6));
        _127.Store<half>(((b_idx + l) + 0u) * 2 + 0, half(d * (float((vui & 15u) | (((qh >> iqs) << uint(4)) & 16u)) - 16.0f)));
        _127.Store<half>(((b_idx + l) + 16u) * 2 + 0, half(d * (float((vui >> uint(4)) | ((qh >> (iqs + 12u)) & 16u)) - 16.0f)));
    }
}
