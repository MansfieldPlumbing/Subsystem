...
Write-Host "  RIFE TRT  -  Release Publisher" -ForegroundColor Cyan
Write-Host "════════════════════════════════════════════════════════════"
...
cmd /c "cd /d `"$ProjectRoot`" && `"$vcvars64`" > nul && cl /LD /O2 /EHsc `"$ProjectRoot\src\rife_trt.cpp`" /I `"$incTrt`" /I `"$incCuda`" /link /LIBPATH:`"$libTrt`" /LIBPATH:`"$libCuda`" nvinfer_10.lib cudart.lib /OUT:`"$ProjectRoot\rife_trt.dll`""
cmd /c "cd /d `"$ProjectRoot`" && `"$vcvars64`" > nul && cl /LD /O2 /EHsc `"$ProjectRoot\src\mf_bridge.cpp`" /OUT:`"$ProjectRoot\mf_bridge.dll`""
