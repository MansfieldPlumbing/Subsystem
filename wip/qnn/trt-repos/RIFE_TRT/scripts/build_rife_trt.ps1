...
if ($dllMissing -gt 0) { Write-Host "`n  ❌ Missing required DLLs. Resolve and re-run Preflight." -ForegroundColor Red; return }
...
if (-not $nvinferLib) { Write-Host "  ❌ nvinfer_10.lib not found."; return }
...
Write-Host "`n  [5.3] Building rife_trt.dll..." -ForegroundColor Yellow
...
if (-not (Test-Path $OutDll)) { Write-Host "  ❌ rife_trt.dll build failed." -ForegroundColor Red; return }

Write-Host "`n  [5.3b] Building mf_bridge.dll..." -ForegroundColor Yellow
$MfSrc = "$ProjectRoot\src\mf_bridge.cpp"
$MfDll = "$ProjectRoot\mf_bridge.dll"
$clCmdMf = "cl /LD /O2 /EHsc `"$MfSrc`" /OUT:`"$MfDll`""
cmd /c "cd /d `"$ProjectRoot`" && `"$vcvars64`" > nul && $clCmdMf"
if (-not (Test-Path $MfDll)) { Write-Host "  ❌ mf_bridge.dll build failed." -ForegroundColor Red; return }

Write-Host "`n  [5.4] Building RIFE_TRT.exe..." -ForegroundColor Yellow
$distDir = "$ProjectRoot\dist"
if (Test-Path $distDir) { Remove-Item $distDir -Recurse -Force }
dotnet publish "$ProjectRoot\src\RIFE_TRT.csproj" -c Release -r win-x64 -o $distDir
if (-not (Test-Path "$distDir\RIFE_TRT.exe")) { Write-Host "  ❌ C# build failed." -ForegroundColor Red; return }
Copy-Item "$distDir\RIFE_TRT.exe" -Destination $ProjectRoot -Force
...
Write-Host "    .\RIFE_TRT.exe `"video.mp4`"" -ForegroundColor White
Write-Host "    .\RIFE_TRT.exe `"video.mp4`" -4x -o smoothed.mp4" -ForegroundColor White
Write-Host ""
