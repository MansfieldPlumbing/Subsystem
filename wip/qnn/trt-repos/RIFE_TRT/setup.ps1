...
if ($PSVersionTable.PSVersion -lt [Version]"7.5") {
    Write-Host "`n  ERROR: PowerShell $($PSVersionTable.PSVersion) detected." -ForegroundColor Red
    Write-Host "  RIFE TRT requires PowerShell 7.5 or later.`n" -ForegroundColor Red
...
$Manifest = [ordered]@{
    ...
    ffmpeg = [PSCustomObject]@{ Label = 'ffmpeg'; MinVersion = [Version]'6.0'; WingetId = 'Gyan.FFmpeg'; WingetArgs = $null; Url = 'https://ffmpeg.org/download.html'; Note = 'required for frame extraction and reassembly' }
...
function Build-Paths {
    ...
    return [PSCustomObject]@{
        ...
        SrcCpp      = "$ProjectRoot\src\rife_trt.cpp"
        OutDll      = "$ProjectRoot\rife_trt.dll"
    }
}
...
function Show-Banner {
    ...
    Write-Host "  ██   RIFE TRT — TensorRT 10.15 / sm86 (Ampere)                      ██" -ForegroundColor Cyan
    Write-Host "  ██                                                                  ██" -ForegroundColor Cyan
    Write-Host "  ██   Part of my sm86 / Ampere TensorRT collection                   ██" -ForegroundColor Cyan
    Write-Host "  ██   https://huggingface.co/MansfieldPlumbing                       ██" -ForegroundColor Cyan
    Write-Host "  ██                                                                  ██" -ForegroundColor Cyan
    Write-Host "  ██   RIFE 4.9 frame interpolation compiled to TensorRT              ██" -ForegroundColor Cyan
    Write-Host "  ██   for native Windows inference.                                  ██" -ForegroundColor Cyan
    Write-Host "  ██                                                                  ██" -ForegroundColor Cyan
    Write-Host "  ██   Validated on RTX 3090 (sm86).                                  ██" -ForegroundColor Cyan
    Write-Host "  ██   Pascal/Blackwell/Hopper builds welcome —                       ██" -ForegroundColor Cyan
    Write-Host "  ██   open an issue with results.                                    ██" -ForegroundColor Cyan
    Write-Host "  ██                                                                  ██" -ForegroundColor Cyan
    Write-Host "  ██   github.com/MansfieldPlumbing                                   ██" -ForegroundColor Cyan
    Write-Host "  ██                                                                  ██" -ForegroundColor Cyan
    Write-Host "  ██████████████████████████████████████████████████████████████████████`n" -ForegroundColor Cyan
}
...
function Show-Menu {
    ...
    $preflightTag = switch ($status) { 'passed' { "  ✅ passed" }; 'failed' { "  ❌ last run failed" }; default { "  ⚠  not yet run" } }
    ...
    Write-Host "    [4]  Python environment     (optional — engine building)" -ForegroundColor DarkGray
...
function Invoke-Python { Write-Host ""; $Paths = Build-Paths; & "$ScriptsDir\setup_python.ps1" -Paths $Paths; Write-Host "`n  Press any key..."; $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown") }

function Invoke-Build {
    if (-not (Test-PreflightPassed)) { Write-Host "`n  ✋ Run [2] Preflight first.`n" -ForegroundColor Red; pause; return }
    Write-Host ""
    $Paths = Build-Paths
    & "$ScriptsDir\build_rife_trt.ps1" -Paths $Paths -DllManifest $DllManifest
