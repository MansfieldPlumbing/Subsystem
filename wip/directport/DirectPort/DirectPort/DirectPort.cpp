...
unsigned long Producer::get_pid() const {
    return pImpl->pid;
}
...
void Window::set_title(const std::string& title) { 
    if (pImpl && pImpl->hwnd) 
        SetWindowTextW(pImpl->hwnd, string_to_wstring(title).c_str()); 
}
...
ID3D11Device* DeviceD3D11::get_d3d11_device() {
    return pImpl->device.Get();
}

ID3D11DeviceContext* DeviceD3D11::get_d3d11_context() {
