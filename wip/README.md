...
each is staged here until it is folded into `src/` (the dpx pattern: vendor → mount → absorb) or, once
...
- **SentencePiece tokenizer** — lifted from `gemma-talking-layer` into `src/native/dpx/onnxnet/SentencePiece.cs`
...
- **gemma4 exporter** → `src/native/dpx/tools/gemma4-export/export_gemma4_e2b_decomposed.py`. A build-time
  re-export recipe ("no python at runtime"); the Gemma-4 read/op-mapping it fed already shipped via the LiteRt
  path in `src/native/dpx`. Kept in the tracked source tree so a clone can re-derive the gated graph.
...
- **`gemma-talking-layer/`** — a bit-exact gemma tokenizer (now graduated, above) + a C#-driven decode loop.
  **Blocked:** the decode loop is wired to a **mock VOM + a stub engine** (fabricated logits); graduates after
  rewiring to the real `dpx.Interp` + real `Subsystem.Vom` behind the `Runtime` contract. CRQ159.
...
- **`dpx-research/`** — the demucs TensorRT case study (external-repo recon). Out of doctrine for `src/`
  (subsystem's dpx is ORT- **and** TRT-free); kept as a tracked case study.
