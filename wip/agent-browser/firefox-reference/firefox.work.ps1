...
param(
    ...
    [string]$Objective = "",
    [switch]$Google,
    [switch]$AsLibrary   # dot-source the functions WITHOUT running the interactive REPL
)
...
(function(){
    try {
        ...
        // EXPLODE IFRAMES: same-origin frames are part of the page; append their text
        // so we see everything. Cross-origin frames (e.g. Google One Tap) throw on
        // access and are skipped (their content is unreachable by design).
        try {
            Array.from(document.querySelectorAll('iframe')).forEach(function(f){
                try {
                    var r = f.getBoundingClientRect();
                    if (r.width < 50 || r.height < 50) return;  // skip 1x1 tracking/Cloudflare iframes
                    var d = f.contentDocument;
                    if (d && d.body) {
                        var ft = (d.body.innerText || "").trim();
                        if (ft) { text += "\n[iframe] " + ft; }
                    }
...
function Parse-And-Display {
    ...
    try {
        ...
        $State.CurrentTab.Text = $text   # carry page text so Get-Hud returns it as an object

        # --- SILENCE ASCII PARSING UI IN GOOGLE MODE ---
        if ($Google) { return }
        
        # CRITICAL FIX: Only clear screen if NOT verbose, otherwise we lose debug info
        # Guard cls: under captured/redirected output there is no real console and
        # Clear-Host throws "handle is invalid", which the outer catch swallows and
        # bails before any page text prints. Skip it when there's no interactive console.
        if (-not $Verbose) {
            try { cls } catch {}
        } else {
...
function Execute-Command {
    ...
    elseif ($Cmd -eq "/forward") {
        $State.DOMActivity.NavigationStarted = $true
        $State.DOMActivity.DOMComplete = $false
        Send-Packet -To $State.CurrentTab.Console -Type "evaluateJSAsync" -Data @{
            text = "window.history.forward()"
        }
        $isNavigation = $true
    }
    elseif ($Cmd -eq "/dismiss") {
        # Remove overlay popups we can't otherwise reach (Google One Tap iframe, consent
        # modals). The popup is a cross-origin iframe, so we remove its container node
        # in the parent DOM, cancel the One Tap API, and Escape. Returns what it hit.
        $js = @"
(function(){
  var hits = [];
  function nuke(s){ document.querySelectorAll(s).forEach(function(e){ hits.push(s); e.remove(); }); }
  // Google One Tap (current + legacy container/iframe ids)
  nuke('iframe[src*="accounts.google.com"]');
  nuke('iframe[src*="/gsi/"]');
  nuke('[id*="credential_picker"]');
  nuke('div[id^="g_id_onload"]');
  nuke('div[class*="g_id"]');
  // generic modal/consent overlays
  nuke('div[aria-modal="true"]');
  nuke('[role="dialog"]');
  nuke('div[class*="onetap"]');
  // anything fixed in a corner holding an iframe (One Tap fallback)
  document.querySelectorAll('div').forEach(function(d){
    try {
      var cs = getComputedStyle(d);
      if (cs.position === 'fixed' && d.querySelector('iframe') && d.getBoundingClientRect().width < 500) {
        hits.push('fixed-iframe'); d.remove();
      }
    } catch(e){}
  });
  try { window.google && google.accounts && google.accounts.id && google.accounts.id.cancel(); } catch(e){}
  try { document.dispatchEvent(new KeyboardEvent('keydown', { key:'Escape', keyCode:27, which:27, bubbles:true })); } catch(e){}
  return 'SWATTED ' + hits.length + (hits.length ? ' :: ' + hits.join(', ') : '');
})()
"@
        $State.LastEvalResult = $null
        Send-Packet -To $State.CurrentTab.Console -Type "evaluateJSAsync" -Data @{ text = $js }
        $sw = Get-Date
        while ((Get-Date) - $sw -lt [TimeSpan]::FromSeconds(3)) {
            Process-BackgroundEvents
            if ($State.LastEvalResult -and $State.LastEvalResult.result) {
                Write-Host "[DISMISS] $(Expand-Grip $State.LastEvalResult.result)" -ForegroundColor Magenta
                break
            }
            Start-Sleep -Milliseconds 50
        }
    ...
    elseif ($Cmd -match "^/(\d+)$") {
        $h = [int]$matches[1]
        # Resolve virtual handle -> real DOM index (falls back to raw index if no map).
        $idx = $h
        if ($State.IntentMap -and $State.IntentMap.Contains($h)) { $idx = [int]$State.IntentMap[$h] }
        $delay = Get-HumanDelay -BaseMs 300

        Write-Host "[ACTION] Clicking handle /$h -> DOM[$idx] (delay: $($delay)ms)..." -ForegroundColor Gray
...
(function(text) {
    ...
    // Prefer a rich-text editor / contenteditable composer (e.g. ChatGPT's .ProseMirror)
    // over hidden/secondary textareas; otherwise the lowest visible input on the page.
    var ces = visible.filter(e => e.isContentEditable);
    var target = ces.length
        ? ces[ces.length - 1]
    ...
    // 2. Insert text. Rich editors (ProseMirror/Lexical, e.g. ChatGPT) ignore raw
    // value/innerText assignment, so drive them via execCommand insertText which fires
    // the proper beforeinput/input pipeline. Plain inputs use the native value setter.
    ...
    // 3. Enter key (search inputs + submit-on-enter editors)
    var prevented = false;
    ['keydown', 'keyup'].forEach(function (t) {
        var ev = new KeyboardEvent(t, { key: 'Enter', code: 'Enter', which: 13, keyCode: 13, bubbles: true });
        if (!target.dispatchEvent(ev)) prevented = true;
    });

    // 4. Explicit send button (ChatGPT etc.), then form submit fallback.
    var sb = document.querySelector('[data-testid="send-button"], button[aria-label*="Send"], button[aria-label*="send"]');
    if (sb && !sb.disabled) {
        sb.click();
    } else if (!prevented && target.form) {
        var submitBtn = target.form.querySelector('[type="submit"]');
        if (submitBtn) { submitBtn.click(); }
        else { target.form.requestSubmit ? target.form.requestSubmit() : target.form.submit(); }
    }
...
# --- PER-PAGE HUD SHAPING (firefox.config.ini) ---
# Loads page identities + intent hide-rules so the HUD shows a friendly page name
# (not the long mutated URL) and drops chrome/noise intents per page.
function Get-PageConfig {
    param([string]$Path)
    $cfg = [ordered]@{}
    ...
    foreach ($raw in (Get-Content -LiteralPath $Path)) {
        ...
        if ($line -match '^\[(.+)\]$') {
            ...
            $cfg[$section] = [ordered]@{ name = $section; match = @(); hide = @(); keep = @(); strip = @(); keep_after = $null; keep_until = $null; max_text = $null }
            continue
        }
        ...
        switch ($k) {
            'name'       { $cfg[$section].name = $v }
            'match'      { $cfg[$section].match += $v }
            'hide'       { $cfg[$section].hide += $v }
            'keep'       { $cfg[$section].keep += $v }       # whitelist: if set, ONLY matching intents show
            'keep_after' { $cfg[$section].keep_after = $v }   # drop everything up to & incl. this match
            'keep_until' { $cfg[$section].keep_until = $v }   # cut text at this match
            'strip'      { $cfg[$section].strip += $v }       # drop any line matching (repeatable)
            'max_text'   { $cfg[$section].max_text = $v }     # cap chars per HUD view; page rest via chunks
        }
    }
    return $cfg
}

# Apply a page's text rules: isolate the high-signal block (e.g. the AI answer).
function Format-HudText {
    param([string]$Text, $Section)
    if (-not $Text -or -not $Section) { return $Text }

    if ($Section.keep_after) {
        $m = [regex]::Match($Text, $Section.keep_after, [Text.RegularExpressions.RegexOptions]::Singleline)
        if ($m.Success) { $Text = $Text.Substring($m.Index + $m.Length) }
    }
    if ($Section.keep_until) {
        $m = [regex]::Match($Text, $Section.keep_until, [Text.RegularExpressions.RegexOptions]::Multiline)
        if ($m.Success) { $Text = $Text.Substring(0, $m.Index) }
    }
    if ($Section.strip) {
        $kept = foreach ($l in ($Text -split "`n")) {
            $drop = $false
            foreach ($s in $Section.strip) { if ($l -match $s) { $drop = $true; break } }
            if (-not $drop) { $l }
        }
        $Text = $kept -join "`n"
    }
...
$script:PageConfig = Get-PageConfig (Join-Path $PSScriptRoot 'firefox.config.ini')

# --- DEVICE CONTEXT (time + where-on-the-globe) ---
# Time is fresh each call; location is resolved once (best-effort, IP-based) and cached.
function Get-DeviceContext {
    if (-not $script:DeviceLoc) {
        $script:DeviceLoc = '(unknown)'
        try {
            $r = Invoke-RestMethod -Uri 'http://ip-api.com/json/?fields=status,country,regionName,city,lat,lon' -TimeoutSec 3
            if ($r.status -eq 'success') {
                $script:DeviceLoc = "$($r.city), $($r.regionName), $($r.country) [$($r.lat), $($r.lon)]"
            }
        } catch {}
    }
    $now = Get-Date
    [PSCustomObject]@{
        ...
        location = $script:DeviceLoc
    }
}

# --- HUD (object, not strings) ---
# Returns the current page as a structured object: friendly page id, state, and a
# filtered list of invokable intents. The raw URL is kept on the object but the
# default render shows `page` instead.
function Get-Hud {
    param([int]$Chunk = 0)   # which slice of a long page to view (see max_text)
    $url = $State.CurrentTab.Url

    # Resolve page identity + rules from config (first matching section wins).
    $pageName = $url
    $hide = @()
    $keep = @()
    $page = $null
    foreach ($sec in $script:PageConfig.Values) {
        $hit = $false
        foreach ($m in $sec.match) { if ($url -match $m) { $hit = $true; break } }
        if ($hit) { $pageName = $sec.name; $hide = $sec.hide; $keep = $sec.keep; $page = $sec; break }
    }

    # Build intents with VIRTUAL HANDLES: after filtering, renumber 1..N so the agent
    # sees clean handles, not raw DOM indices. $State.IntentMap maps handle -> real DOM
    # index so the click command (/N) still lands on the right element.
    $intents = New-Object System.Collections.Generic.List[object]
    $map = @{}
    $handle = 0
    foreach ($line in ($State.CachedElements -split "`n")) {
        $p = $line -split "\|"
        if ($p.Count -ne 3) { continue }
        $label = $p[2]
        $skip = $false
        foreach ($h in $hide) { if ($label -like $h) { $skip = $true; break } }
        if ($skip) { continue }
        # Whitelist: if `keep` is set, an intent must match at least one pattern.
        if ($keep.Count) {
            $ok = $false
            foreach ($k in $keep) { if ($label -like $k) { $ok = $true; break } }
            if (-not $ok) { continue }
        }
        $handle++
        $dom = [int]$p[0]
    ...
    # Shape text, then page it so long sites don't burn tokens. max_text (per page or
    # global default) caps each view; /nextchunk advances $Chunk.
    $fullText = Format-HudText -Text $State.CurrentTab.Text -Section $page
    $max = 0
    if ($page -and $page.max_text) { $max = [int]$page.max_text }
    elseif ($script:PageConfig['*'] -and $script:PageConfig['*'].max_text) { $max = [int]$script:PageConfig['*'].max_text }

    $chunkInfo = $null
    $text = $fullText
    if ($max -gt 0 -and $fullText.Length -gt $max) {
        $total = [Math]::Ceiling($fullText.Length / $max)
        if ($Chunk -lt 0) { $Chunk = 0 }
        if ($Chunk -ge $total) { $Chunk = $total - 1 }
        $start = $Chunk * $max
        $len = [Math]::Min($max, $fullText.Length - $start)
        $text = $fullText.Substring($start, $len)
        $more = if ($Chunk + 1 -lt $total) { " — /nextchunk for more" } else { " — end" }
        $chunkInfo = "[chunk $($Chunk + 1)/$total$more]"
    }
...
# --- LIBRARY MODE: stop here so a dot-source gets the functions, not the REPL ---
if ($AsLibrary) { return }
